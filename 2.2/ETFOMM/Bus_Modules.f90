...
! along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  MODULE BUS_ROUTE_DATA    
    ...
    LOGICAL :: TRAM_SIMULATION = .FALSE.
    INTEGER, ALLOCATABLE :: BUSR_HDWY(:)
    INTEGER, ALLOCATABLE :: BUSR_TRAMCARS(:)
    ...
    REAL,    ALLOCATABLE :: BUSR_TRAVELDIST(:)
    REAL,    ALLOCATABLE :: BUSR_DWELLTIME(:)
    REAL,    ALLOCATABLE :: BUSR_CONTROLDELAY(:)
    REAL,    ALLOCATABLE :: BUSR_DIST(:)
...
! ==================================================================================================
    ...
    IF(.NOT. ALLOCATED(BUSR_ROUTE_NODES)) THEN
      ...
      ALLOCATE(BUSR_TRAMCARS(NUMBER_OF_ROUTES))
      ...
      ALLOCATE(BUSR_TRAVELDIST(NUMBER_OF_ROUTES))
      ALLOCATE(BUSR_DWELLTIME(NUMBER_OF_ROUTES))
      ALLOCATE(BUSR_CONTROLDELAY(NUMBER_OF_ROUTES))
      ALLOCATE(BUSR_DIST(NUMBER_OF_ROUTES))
...
! ==================================================================================================
    ...
    IF(ALLOCATED(BUSR_TRAMCARS)) DEALLOCATE(BUSR_TRAMCARS)
    ...
    IF(ALLOCATED(BUSR_TRAVELDIST)) DEALLOCATE(BUSR_TRAVELDIST)
    IF(ALLOCATED(BUSR_DWELLTIME)) DEALLOCATE(BUSR_DWELLTIME)
    IF(ALLOCATED(BUSR_CONTROLDELAY)) DEALLOCATE(BUSR_CONTROLDELAY)
    IF(ALLOCATED(BUSR_DIST)) DEALLOCATE(BUSR_DIST)
...
! --- Save static data.
    ...
    DO I = 1, NUMBER_OF_ROUTES 
      ...
      WRITE(FILE) BUSR_TRAMCARS(I)
      WRITE(FILE) BUSR_TIMER(I)
      WRITE(FILE) BUSR_OFFSET(I)
      WRITE(FILE) (BUSR_STATIONLIST(I, N), N = 1, 100)
      WRITE(FILE) BUSR_TRAVELTIME(I)
      WRITE(FILE) BUSR_TRAVELDIST(I)
      WRITE(FILE) BUSR_DWELLTIME(I)
      WRITE(FILE) BUSR_CONTROLDELAY(I)
      WRITE(FILE) BUSR_DIST(I)
...
! --- Restore data.
    ...
    IF(NUMBER_OF_ROUTES .NE. 0) THEN
      ...
      DO I = 1, NUMBER_OF_ROUTES 
        ...
        READ(FILE) BUSR_TRAMCARS(I)
        READ(FILE) BUSR_TIMER(I)
        READ(FILE) BUSR_OFFSET(I)
        READ(FILE) (BUSR_STATIONLIST(I, N), N = 1, 100)
        READ(FILE) BUSR_TRAVELTIME(I)
        READ(FILE) BUSR_TRAVELDIST(I)
        READ(FILE) BUSR_DWELLTIME(I)
        READ(FILE) BUSR_CONTROLDELAY(I)
        READ(FILE) BUSR_DIST(I)
  ...
  MODULE BUS_STATION_DATA
    ...
    TYPE BUS_STATION
      ...
      INTEGER :: LINK = 0
      INTEGER :: LOCATION = 0
      INTEGER :: ROUTE_LOCATION = 0
      INTEGER :: APPROACH_DISTANCE = 0
      INTEGER :: NEXT_STATION = 0
      REAL    :: OVERFLOW_TIME = 0
      INTEGER :: POCKET_LANE = 0
      INTEGER :: TYPE_CODE = 0
      REAL    :: TIME_FROM_LAST = 0
      REAL    :: DISTANCE_FROM_LAST_STATION = 0
    END TYPE
...
! ==================================================================================================
    SUBROUTINE ALLOCATE_BUS_STATION_ARRAYS(N)
    INTEGER :: N
    IF(.NOT. ALLOCATED(BUS_STATION_LIST)) THEN
      ALLOCATE(BUS_STATION_LIST(N))
      ALLOCATE(WRITE186(N, 19))
      ALLOCATE(STATION_DWELL(N, 19))
      ALLOCATE(STATION_BYPASS_PCT(N, 19))
      WRITE186 = .FALSE.
      STATION_DWELL = 0.
