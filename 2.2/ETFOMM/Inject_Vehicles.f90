...
! ----------------------------------------------------------------------      
  ...
  TYPE(VEHICLE), INTENT(INOUT) :: THIS_VEHICLE
  LOGICAL, INTENT(OUT) :: GOOD
  INTEGER :: IV, I, ILD, IFLT, ITYPE, CFM, ICAR, VID
  REAL :: ACCEL, RNDNUM
  LOGICAL :: IW
  COMMON /GRAPHIO/ TSTEPFILE, TINTERVALFILE
  INTEGER :: TSTEPFILE, TINTERVALFILE
...
! --- Find the first available index into the vehicle arrays.
  ...
  IF(THIS_VEHICLE%ROUTEID .NE. 0) THEN
    ...
    SPATHPOINT(IV) = 1
    NEXT_STOP(IV) = 1
    IF(BUSR_TRAMCARS(THIS_VEHICLE%ROUTEID) .EQ. 0) THEN
      CALL BUS_PROPERTIES_STREET(IFLT, ITYPE)
    ELSE
      CALL TRAM_PROPERTIES_STREET(IFLT, ITYPE)
    ENDIF
    SFLEET(IV) = IFLT
    SVTYPE(IV) = ITYPE
...
! --- The vehicle must enter at a lower speed.
    ...
    TYPE_IS_USED(SVTYPE(IV)) = .TRUE.
    TRAM_FOLLOWER(IV) = .FALSE.
    IF(THIS_VEHICLE%ROUTEID .NE. 0) THEN
      IF(BUSR_TRAMCARS(THIS_VEHICLE%ROUTEID) .GT. 1) THEN
        DO ICAR = 2, BUSR_TRAMCARS(THIS_VEHICLE%ROUTEID)
          ILD = IV
          CALL GET_NEXT_SINDEX(IV)
          CALL DELETE_STREET_VEHICLE(IV, .FALSE.)
          CALL GET_NEXT_ID(VID)
          DRIVER_TYPE(IV) = SDRIVERTYPE(ILD)
          TRAM_FOLLOWER(IV) = .TRUE.
          TRAM_LEADER(IV) = ILD
          SROUTEID(IV) = SROUTEID(ILD)
          STURNCODE(IV) = STURNCODE(ILD)
          SLANECODES(IV, 1) = LC_GOOD
          SDESIREDSPEED(IV) = SDESIREDSPEED(ILD)
          SSPEED(IV) = SSPEED(ILD)
          SCF_MODEL(IV) = SCF_MODEL(ILD)
          NEXT_STOP(IV) = 1
          SID(IV) = VID
          SLINK(IV) = SLINK(ILD)
          SLANE(IV) = SLANE(ILD)
          SVLENGTH(IV) = SVLENGTH(ILD)
          SDRIVERTYPE(IV) = SDRIVERTYPE(ILD)
          SFLEET(IV) = SFLEET(ILD)
          SVTYPE(IV) = SVTYPE(ILD)
          SLEADER(IV) = ILD
          SFOLLOWER(ILD) = IV
          SLAST_VEHICLE(SLINK(IV), SLANE(IV)) = IV
          SLOCATION(IV) = SLOCATION(ILD) - SVLENGTH(ILD)
          THIS_VEHICLE%DRIVER = SDRIVERTYPE(IV)
          THIS_VEHICLE%FLEET = SFLEET(IV)
          THIS_VEHICLE%VTYPE = SVTYPE(IV)
          IF(TSTEPFILE .EQ. 2) CALL WRITE_VEHICLE_LIST(THIS_VEHICLE, SUSN(SLINK(IV)), SVLENGTH(IV))      
...
! ----------------------------------------------------------------------
  ...
  !LZ Changed 5 to 20 tempraraly
  INTEGER :: I, IL, N, LEAD(20), IPX, ILAST
  REAL :: RNDNUM, HEADWAY(20), HMAX, EPCT
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
  temp = fid(iv)
...
! ----------------------------------------------------------------------
  ...
  INTEGER, INTENT(IN) :: IV
  INTEGER, INTENT(OUT) :: ILD
  INTEGER :: I, IL, N, LEAD(20), IUP, ILANE
  REAL :: RNDNUM, HEADWAY(20), HMAX, EPCT
! ----------------------------------------------------------------------
  SLANE(IV) = 0
  IL = SLINK(IV)
...
! --- Loop over the lanes on the entry link.
  ...
  !If the vehicle will make a turn bias the entry lane to left or right
  I = 0
  IF(TURN_CODE(IV) .EQ. TC_LEFT .OR. TURN_CODE(IV) .EQ. TC_LDIAG) THEN
    I = LAST_FULL_LANE(IL)
  ELSEIF(TURN_CODE(IV) .EQ. TC_RIGHT .OR. TURN_CODE(IV) .EQ. TC_RDIAG) THEN
    I = 1
  ENDIF
  IF(I .NE. 0) THEN
...
! --- If the headway is sufficient choose this lane.
    ...
    ELSEIF(SLINK(ILD) .EQ. STHRU_LINK(IL)) THEN
      IF(SLOCATION(ILD) + SSPEED(ILD) * TIMESTEP .LE. SVLENGTH(ILD)) THEN
        HEADWAY(I) = 0.
      ELSE
...
! ==================================================================================================
  SUBROUTINE TRAM_PROPERTIES_STREET(IFLT, ITYPE)
! ----------------------------------------------------------------------
! --- Determine tram properties.
! ----------------------------------------------------------------------
  USE VEHICLE_TYPES
  USE STREET_VEHICLES
  USE SIMPARAMS
