...
! ----------------------------------------------------------------------
  ...
  INTEGER :: IL, ILANE, IGOAL, ITURN, ICODE, NLC
  INTEGER :: RLANE
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
...
! --- as the goal lane.
        ...
        IGOAL = RLANE
      ELSE
        CALL FIND_TARGET_LANE_FREEWAY(IV, IGOAL, NLC)
        !If the vehicle is close to the required lane change point and has to make
        !more than one lane change, let it use the current lane
        IF(NLC .GT. 1 .AND. REMAINING_DIST(IV) .NE. 0.) THEN
          IF(REMAINING_DIST(IV) .LT. 50.) THEN
            IGOAL = 0
            IF(ICODE .EQ. LC_VACATE) FLANECODES(IV, ILANE) = LC_GOOD
          ENDIF
        ENDIF
      ENDIF
...
! ==================================================================================================
  SUBROUTINE FIND_TARGET_LANE_FREEWAY(IV, GOAL, NLC)
! ----------------------------------------------------------------------
...
! --- new segment look for a follower on the last link of the previous segment.
  ...
  TRY = .TRUE.
  IF(ILD .NE. 0) THEN
    IF(DISTANCE_TO_SEGMENT_END(ILD) + FVLENGTH(ILD) .GT. DISTANCE_TO_SEGMENT_END(IV) + FSPEED(IV) * TIMESTEP) TRY = .FALSE.
  ENDIF
  IF(TRY .AND. IFL .NE. 0) THEN
    IF(DISTANCE_TO_SEGMENT_END(IV) + FVLENGTH(IV) .GT. DISTANCE_TO_SEGMENT_END(IFL) + FSPEED(IFL) * TIMESTEP) TRY = .FALSE.
  ENDIF
  IF(TRY .AND. ILD .NE. 0 .AND. IFL .NE. 0) THEN
    IF(DISTANCE_TO_SEGMENT_END(IFL) - DISTANCE_TO_SEGMENT_END(ILD) - FVLENGTH(ILD) .LT. 2 * FVLENGTH(IV)) TRY = .FALSE.
  ENDIF
  IF(TRY .AND. (ILD .NE. 0 .OR. IFL .NE. 0) .AND. RAMPMETER(FLINK(IV)) .NE. 0) THEN
    IF(FLENGTH(IL) - FLOCATION(IV) .LT. 50) TRY = .FALSE.    !!!!arbitrary constant
  ENDIF
  AD = 1000.
  PD = 0.
  IF(TRY) THEN
...
! --- Determine if the follower will cooperate by slowing down.
    ...
    IF(FWILL_COOP_LC(IFL) .OR. INCIDENT_NUM(IFL) .NE. 0 .OR. FLANE(IV) .EQ. 0 .OR. &
      ...
      CALL FREEWAY_CAR_FOLLOW(IFL, IV, PD, DISTANCE)
...
! --- If the potential deceleration is small tell the follower to slow down.
  ...
  ELSEIF(ILD .NE. 0) THEN
    IF(FDIVERTED(IV)) THEN
      CALL FREEWAY_CAR_FOLLOW(IV, ILD, PD, DISTANCE)
...
! ----------------------------------------------------------------------
  ...
  REAL, INTENT(OUT) :: D
  REAL D1, D2
  REAL :: DISTANCE, STOPDIST, RMAX, DIST1, DIST2
! ----------------------------------------------------------------------
  D1 = 0.
  D2 = 0.
  DIST1 = 100.
...
! --- vehicle and the putative follower if the lane change is attempted.
  ...
  IF(ILD .NE. 0) THEN
    CALL FREEWAY_CAR_FOLLOW(IV, ILD, D1, DISTANCE)
    DIST1 = DISTANCE
  ENDIF
  IF(IFL .NE. 0) THEN
    CALL FREEWAY_CAR_FOLLOW(IFL, IV, D2, DISTANCE)
    DIST2 = DISTANCE
  ENDIF
  IF(DIST1 + DIST2 .GT. 100) THEN
    IF(D1 .LT. 0.) THEN
      RMAX = MAX_DECEL(FVTYPE(IV), FSPEED(IV), FGRADE(FLINK(IV)))
      STOPDIST = -FSPEED(IV) ** 2 / (2 * RMAX)
      IF(STOPDIST .LT. DIST1) THEN
        D1 = 0.
      ENDIF
    ENDIF
    IF(D2 .LT. 0.) THEN
      RMAX = MAX_DECEL(FVTYPE(IFL), FSPEED(IFL), FGRADE(FLINK(IFL)))
      STOPDIST = -FSPEED(IFL) ** 2 / (2 * RMAX)
      IF(STOPDIST .LT. DIST2) THEN
        D2 = 0.
      ENDIF
    ENDIF
  ENDIF
...
! ----------------------------------------------------------------------
  ...
  INTEGER, INTENT(IN) :: IV
  INTEGER :: LEFT, RIGHT, GOAL, IL
  REAL :: RNDNUM, TLRSP, DESIRE, HDWY, HDWY_LEFT, HDWY_RIGHT, XMIN
  !!!!! HMIN does not relate to any CORSIM input
  !!!!! It is the minimum headway difference to trigger a lane change
  !!!!! CORSIM uses an advantage factor
  !!!!! This is an arbitrary number used for testing.
...
! --- Determine if there is an advantage in moving into the left or right lane.
    ...
    XMIN = 0.
    IF(IL .EQ. TURN_LINK(IV) .OR. STHRU_LINK(IL) .EQ. TURN_LINK(IV)) THEN
      XMIN = HMIN
    ENDIF
    IF(MAX(HDWY_LEFT, HDWY_RIGHT) - HDWY .GT. HMIN + XMIN) THEN
      IF(HDWY_LEFT .GT. HDWY_RIGHT) THEN
        GOAL = LEFT
      ELSEIF(HDWY_LEFT .LT. HDWY_RIGHT) THEN
        GOAL = RIGHT
...
! --- Determine if the subject vehicle can enter the turning way.
  ...
  IL = SLINK(IV)
  IF(SPREVLANE(IV) .EQ. FIRST_FULL_LANE(IL) .AND. SLOCATION(IV) .GE. RTW_EXIT_POINT(IL)) THEN
    ...
    ENDIF
  ELSEIF(SLOCATION(IV) .GE. RTW_EXIT_POINT(IL) + 20 * RTW_LANES(IL)) THEN
    IN_TURNING_WAY(IV) = .FALSE.
