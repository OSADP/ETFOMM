...
! ----------------------------------------------------------------------
  ...
  SIGNPOS = MAXPOS
  IF(SIGNPOS .GE. USN_TO_SEG_END(IL) + HOV_WARN(IL)) THEN
    SIGNPOS = USN_TO_SEG_END(IL) + HOV_WARN(IL)
  ENDIF
  HOV_WARN(IL) = SIGNPOS

! --- Check if there are any on-ramps between this HOV facility and its
...
! --- Set the warning sign on the approach link.

  INODE = FDSN(IL)
  SIGNPOS = HOV_WARN(IL)
  IF(SIGNPOS .EQ. MAXPOS) SIGNPOS = SIGNPOS - 1
  IF(MAINLINE_APPROACH(INODE) .NE. 0) THEN
    CALL LOAD_OBJECT(ISEG, MAINLINE_APPROACH(INODE), 0, 0, SIGNPOS, NOBJECTS, M_HOV_WARN)
  ELSEIF(RAMP_APPROACH(INODE) .NE. 0) THEN
    CALL LOAD_OBJECT(ISEG, RAMP_APPROACH(INODE), 0, 0, SIGNPOS, NOBJECTS, M_HOV_WARN)
  ENDIF
  RETURN
  END
...
! --- Set the warning sign on the main line.
        ...
        DO
          ...
          ELSE
            ...
            IF(MAINLINE_APPROACH(FUSN(ULINK)) .EQ. 0) EXIT
            IF(NODE_TYPE(FUSN(MAINLINE_APPROACH(FUSN(ULINK)))) .EQ. NT_EXTERN) EXIT
          ENDIF
        ENDDO
        CALL LOAD_OBJECT(ISEG, ULINK, TRUCK_LANE(IL), IL, ZPOS, NOBJECTS, M_ETL_WARN)
      ENDIF
    ENDIF
  ENDDO
  RETURN
...
! --- Mainline link to offramp link.
          ...
          CALL LOAD_NODE_ALIGNMENT(ULINK, FTHRU_LINK(ULINK), NOBJECTS, ISEG)
        ENDIF
      ENDIF
    ENDIF
  ENDDO
...
! ----------------------------------------------------------------------
  ...
  ELSE
    ...
    IF(OAL .LT. ULANES .OR. OAL .GT. 15) THEN
      DO I = 2, DLANES
        IF(OAL .GT. 15) THEN
      ...
      ENDDO
    ELSE
      !Consider left-side full lanes
      I = OFFRAMP_RECEIVING_LANE(ULINK)
      IAUX = 0
      DO I = OAL + 1, FNUMLANES(ULINK)
        ILANE = OFFRAMP_RECEIVING_LANE(ULINK) + 1
        IF(ILANE .LE. FNUMLANES(DLINK)) THEN
          EXIT_LANE(ULINK, I) = ILANE
        ELSE
          IAUX = IAUX + 1
          EXIT_LANE(ULINK, I) = AUX_LANE_ID(ULINK, IAUX)
        ENDIF
      ENDDO
...
! ----------------------------------------------------------------------
  ...
  INTEGER :: NLUP, NLDN, FLUP, FLDN
  INTEGER :: IUP, IDOWN, SHIFT, IL
! ----------------------------------------------------------------------
#ifdef DebugVersion
  integer :: temp
  temp = fusn(ulink)
...
! --- Store the downstream lanes that receive traffic from the upstream lanes.
  ...
  IF(OFFRAMP_LINK(ULINK) .NE. 0) THEN
    IF(OFFRAMP_SENDING_LANE(ULINK) .NE. 1) THEN
      FLDN = FLDN + FNUMLANES(OFFRAMP_LINK(ULINK))
    ENDIF
  ENDIF
  
  IF(NODE_TYPE(FDSN(DLINK)) .NE. NT_EXTERN) THEN
    IF(FLUP .NE. FLDN) THEN
      IF(FLDN .GT. FLUP) THEN
        DO IL = 1, N_FREEWAY_LINKS
          IF(IL .EQ. ULINK .OR. IL .EQ. DLINK) CYCLE
          IF(FTHRU_LINK(IL) .EQ. DLINK) THEN
            FLUP = FLUP + FNUMLANES(IL)
            DO I = 1, 3
              FLUP = FLUP + ADDDROP_CODE(IL, I)
            ENDDO
            EXIT
          ENDIF
        ENDDO
      ENDIF
      IF(FLUP .NE. FLDN) THEN
        WRITE(MSGTEXT, '(A, I6)') 'THE NUMBER OF INCOMING LANES DOES NOT MATCH THE NUMBER OF OUTGOING LANES AT NODE ', FDSN(ULINK)
        CALL SENDTEXTMSG(M_ERROR)
        WRITE(MSGTEXT, '(A, I6)') ' THE NUMBER OF INCOMING LANES = ', FLUP
        CALL SENDTEXTMSG(M_ERROR)
        WRITE(MSGTEXT, '(A, I6)') ' THE NUMBER OF OUTGOING LANES = ', FLDN
        CALL SENDTEXTMSG(M_ERROR)
      ENDIF
    ENDIF
  ENDIF
