...
!----------------------------------------------------------------------
  ...
  INTEGER :: INDEX, PRIORITY_INDEX, PRIORITY_PAIR, IMODE, PHASE, ILINK, IL
...
!----------------------------------------------------------------------
  ...
  DO PHASE = 1, 8
    ...
    IMODE = AC_SIGNALS(IACT)%GUI_ACTUATED_MODE(PHASE)
    IF(IMODE .EQ. PRESENCE_MODE .OR. IMODE .EQ. RECALL_MIN) THEN
      AC_SIGNALS(IACT)%SDP%GREEN_TIME(PHASE) = AC_SIGNALS(IACT)%GUI_MIN_GREEN_TIMES(PHASE)
    ELSEIF(IMODE .EQ. RECALL_MAX) THEN
      AC_SIGNALS(IACT)%SDP%GREEN_TIME(PHASE) = AC_SIGNALS(IACT)%GUI_MAX_GREEN_TIMES(PHASE)
    ENDIF
    AC_SIGNALS(IACT)%SDP%TIME_LIMIT(PHASE) = AC_SIGNALS(IACT)%SDP%GREEN_TIME(PHASE) + AC_SIGNALS(IACT)%GUI_RC(PHASE) + AC_SIGNALS(IACT)%GUI_YC(PHASE)
    
    !Identify phases that serve trams or buses with priority
    do ilink = 1, ac_signals(iact)%n_direct_approaches
      il = ac_signals(iact)%direct_approach(ilink)
      if(ac_signals(iact)%checkin_distance(phase, ilink) .gt. 0) then
        if(ac_signals(iact)%phase(phase)%thruarrow(ilink)) then
          ac_signals(iact)%tram_phase(phase) = .true.
          if(tram_link(il)) ac_signals(iact)%tram_only_phase(phase) = .true.
          priority_link_phase(il) = phase
        endif
      endif
    enddo
    !If a tram phase also serves other approaches consider it a shared phase instead of a tram phase
    do ilink = 1, ac_signals(iact)%n_direct_approaches
      il = ac_signals(iact)%direct_approach(ilink)
      if(.not. tram_link(il)) then
        if(ac_signals(iact)%phase(phase)%thruarrow(ilink)) then
          ac_signals(iact)%tram_only_phase(phase) = .false.
        endif
      endif
    enddo
  ENDDO
  ...
  DO INDEX = 1, 8
    ...
    IF(IMODE .EQ. RECALL_MIN .OR. IMODE .EQ. RECALL_MAX) THEN
      ...
      IF(PING_PONG_FLAG .AND. PRIORITY_INDEX .NE. 0 .AND. PRIORITY_PAIR .NE. 0) THEN
...
!----------------------------------------------------------------------
  ...
  INTEGER :: OTHER_PHASE, MASTER_PHASE, I, N1, N2, policy
...
! ---------------------------------------------------------------------- 
  ...
  DO DETECT_INDEX = 1, AC_SIGNALS(IACT)%SDP%DETECTOR_COUNT
    ...
    IF(CHANNEL_HAS_NEW_CALL) THEN 
      ...
      if(using_dual_entry) then
        if(ac_signals(iact)%dual_entry_code(calling_phase) .eq. 1) then
          ac_signals(iact)%has_demand(paired_phase(calling_phase)) = .true.
        endif
      endif
  ...
  DO IRING = RING_1, RING_2
    ...
    IF(PHASE .NE. 0) THEN
      ...
      ELSE
        ...
        elseif(use_tram_dcs) then
          if(btest(ac_signals(iact)%extension_flags, phase-1)) then
            waitforgap = .true.
          elseif(btest(ac_signals(iact)%forceoff_flags, phase-1)) then
            if(ac_signals(iact)%sdp%phase_timer(phase) .ge. ac_signals(iact)%gui_min_green_times(phase)) then
              ac_signals(iact)%forceoff_flags = ibclr(ac_signals(iact)%forceoff_flags, phase-1)
              if(phase_is_green(iact, phase)) then
                !set parameters to force the phase to go into yellow
                ac_signals(iact)%sdp%wait_for_gap(phase) = .false.
                ac_signals(iact)%sdp%phase_timer(phase) = time_limit - ac_signals(iact)%sdp%rc(phase) - ac_signals(iact)%sdp%yc(phase)      
          ...
          elseif(ac_signals(iact)%hold_for_tram) then
              waitforgap = .true.
          elseif(ac_signals(iact)%serving_tram(phase)) then   
            if(time_in_phase .ge. time_limit - ac_signals(iact)%sdp%yc(phase) - ac_signals(iact)%sdp%rc(phase) .and. .not. waitforgap) then
              if(phase_is_green(iact, phase)) then
                !set parameters to force the phase to go into yellow
                ac_signals(iact)%sdp%phase_timer(phase) = time_limit - ac_signals(iact)%sdp%rc(phase) - ac_signals(iact)%sdp%yc(phase)      
                ac_signals(iact)%sdp%phase_counter(phase) = ac_signals(iact)%sdp%phase_timer(phase) / scope_timestep
                time_in_phase = time_limit - ac_signals(iact)%sdp%yc(phase) - ac_signals(iact)%sdp%rc(phase)
                force_yellow = .true.
              endif
            endif
          endif
...
call sendtextmsg(m_info)
      ...
      IF(TIME_IN_PHASE .GE. TIME_LIMIT .AND. .NOT. WAITFORGAP) THEN
        ...
        ELSE
          CALL ACTUATED_PHASE_COMPLETED(IACT, RING, PHASE)
          ac_signals(iact)%serving_tram(phase) = .false.
        ENDIF
      ...
      ELSEIF(FORCE_YELLOW .OR. (TIME_IN_PHASE .GE. TIME_LIMIT - AC_SIGNALS(IACT)%SDP%YC(PHASE) - AC_SIGNALS(IACT)%SDP%RC(PHASE) .AND. .NOT. WAITFORGAP)) THEN
        ...
        !Force off non-priority phases if the tram that checked in last was not able to checkout
        !and the policy includes force off
        if(ac_signals(iact)%serving_tram(phase)) then
          if(ac_signals(iact)%last_tram_car(phase) .ne. 0) then
            if(.not. last_car_checked_out(ac_signals(iact)%last_tram_car(phase))) then
              policy = ac_signals(iact)%checkin_policy(phase)
              if(policy .eq. 4 .or. policy .eq. 6 .or. policy .eq. 12 .or. policy .eq. 14) then
                !Reset for the next cycle
                ac_signals(iact)%serving_tram(phase) = .true.
                do i = 1, 8
                  if(ac_signals(iact)%phase(i)%in_use .and. .not. ac_signals(iact)%tram_phase(i)) then
                    ac_signals(iact)%forceoff_flags = ibset(ac_signals(iact)%forceoff_flags, i-1)
                  endif
                enddo
              endif
            endif
  ...
  IF(SUM(AC_SIGNALS(IACT)%CURRENT_PHASES) .EQ. 0) THEN
    ...
    IF(AC_SIGNALS(IACT)%READY_TO_CROSS(1) .AND. AC_SIGNALS(IACT)%READY_TO_CROSS(2)) THEN
      ...
      if(sum(ac_signals(iact)%next_tram_phase) .ne. 0) then
        do ring = 1, 2
          next_phase = ac_signals(iact)%next_tram_phase(ring)
          if(next_phase .ne. 0) then
            call start_actuated_phase(iact, ring, next_phase)
            ac_signals(iact)%extension_flags = ibset(ac_signals(iact)%extension_flags, next_phase - 1)
          endif
          ac_signals(iact)%next_tram_phase(ring) = 0
        enddo
      else
        if(sum(ac_signals(iact)%tram_reset_index) .ne. 0) then
          !Try to restart the phases that were terminated for the tram.
          do ring = 1, 2
            index = ac_signals(iact)%tram_reset_index(ring)
            if(index .ne. 0) then
          ...
          IF(AC_SIGNALS(IACT)%MAX_INDEX(1, 1) .NE. 0) THEN
            ...
            ENDIF
          ELSE
            IF(INDEX .LE. AC_SIGNALS(IACT)%MAX_INDEX(2, 1)) THEN
              INDEX = 1
            ELSE
              INDEX = AC_SIGNALS(IACT)%MAX_INDEX(2, 1) + 1
            ENDIF
          ENDIF
          ...
          DO RING = 1, 2
            NEXT_PHASE = AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(RING, INDEX)
            IF(NEXT_PHASE .NE. 0) THEN
              IF(.NOT. AC_SIGNALS(IACT)%PHASE(NEXT_PHASE)%IN_USE) CYCLE
      ...
      IF(SUM(AC_SIGNALS(IACT)%CURRENT_PHASES) .EQ. 0) THEN
        ...
        DO RING = 1, 2
          ...
          ELSE
            IF(AC_SIGNALS(IACT)%PHASE(NEXT_PHASE)%IN_USE) THEN
              IF(PHASE_IS_ALLOWED(IACT, NEXT_PHASE)) CALL START_ACTUATED_PHASE(IACT, RING, NEXT_PHASE)
            ENDIF
          ENDIF
        ENDDO
  ...
  ENDDO
  IF(NPHASE .EQ. 1) THEN
    IF(.NOT. AC_SIGNALS(IACT)%tram_only_phase(OTHER_PHASE)) THEN
      RING = THE_OTHER_RING(IRING)
      IF(AC_SIGNALS(IACT)%NEXT_PHASE(RING) .NE. 0) THEN
        NEXT_PHASE = AC_SIGNALS(IACT)%NEXT_PHASE(RING)
        IF(.NOT. CONFLICTS(OTHER_PHASE, NEXT_PHASE) .AND. PHASE_IS_ALLOWED(IACT, NEXT_PHASE)) THEN
...
!----------------------------------------------------------------------
  ...
  ELSE
    DO INDEX = 1, 4
      PHASE = AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(1, INDEX)
      AC_SIGNALS(IACT)%RING_INDEX(1) = INDEX
      IF(PHASE .NE. 0) THEN
        IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) THEN
    ...
    ENDDO
    DO INDEX = 1, 4
      PHASE = AC_SIGNALS(IACT)%RING_PHASE_SEQUENCE(2, INDEX)
      AC_SIGNALS(IACT)%RING_INDEX(2) = INDEX
      IF(PHASE .NE. 0) THEN
        IF(AC_SIGNALS(IACT)%PHASE(PHASE)%IN_USE) THEN
...
!----------------------------------------------------------------------
  ...
  !If a phase that doesn't serve the tram is started, clear the phase to reset after
  !the tram preemption. The preemption is over, or the tram is waiting through this phase. 
  if(ac_signals(iact)%tram_reset_index(ring) .ne. 0) then
    if(.not. ac_signals(iact)%tram_only_phase(phase)) then
      ac_signals(iact)%tram_reset_index(ring) = 0
    endif
  endif
  
  !Clear the tram force off flags when the tram phase has been reached
  if(use_tram_dcs .and. ac_signals(iact)%forceoff_flags .ne. 0) then
    if(ac_signals(iact)%tram_phase(phase)) then
      ac_signals(iact)%forceoff_flags = 0
    endif
  endif
  ...
  if(ac_signals(iact)%tram_only_phase(phase)) then
    if(.not. initmode .or. simtime .gt. 0) then
      ac_signals(iact)%locked(ring) = .true.
    endif
  endif
...
call sendtextmsg(m_info)
  ...
  if(sum(ac_signals(iact)%next_tram_phase) .ne. 0) then
    if(ring .eq. 1) then
      indx = ac_signals(iact)%next_tram_phase(ring)
    else
      indx = ac_signals(iact)%next_tram_phase(ring) - 4
    endif
  else
  !Determine if the ring is ready to cross the barrier.
    ...
    IF(INDX .EQ. AC_SIGNALS(IACT)%MAX_INDEX(RING, 1) .OR. INDX .EQ. AC_SIGNALS(IACT)%MAX_INDEX(RING, 2)) THEN
      ...
      ENDIF
      IF(INDX .EQ. AC_SIGNALS(IACT)%MAX_INDEX(RING, 1)) THEN
        INDX = 3
      ELSE
        INDX = 1
      ENDIF
...
!-------------------------------------------------------------------------
  ...
  INTEGER :: I, IP, PHASE1, PHASE2, INDEX, THIS_RING, N1, N2, IMODE
  REAL :: REMAINING_GREEN_TIME
...
!-------------------------------------------------------------------------
  ...
  ELSE
    ...
    IF(PHASE1 .NE. 0) THEN
      ...
      IMODE = AC_SIGNALS(IACT)%SDP%ACTUATED_MODE(PHASE1)
      IF((IMODE .EQ. RECALL_MIN .OR. IMODE .EQ. RECALL_MAX) .AND. &
          .NOT. AC_SIGNALS(IACT)%SERVED_THIS_CYCLE(PHASE1)) THEN 
        QUEUED1 = .TRUE.
      ENDIF
    ENDIF
    QUEUED2 = .FALSE.
    IF(PHASE2 .NE. 0) THEN
      ...
      IMODE = AC_SIGNALS(IACT)%SDP%ACTUATED_MODE(PHASE2)
      IF((IMODE .EQ. RECALL_MIN .OR. IMODE .EQ. RECALL_MAX) .AND. &
          .NOT. AC_SIGNALS(IACT)%SERVED_THIS_CYCLE(PHASE2)) THEN 
        QUEUED2 = .TRUE.
      ENDIF
    ENDIF
...
!-------------------------------------------------------------------------
  DO PHASE = 1, 8
    IF(AC_SIGNALS(IACT)%PED_PHASE(PHASE)) THEN
      ...
      ELSEIF(AC_SIGNALS(IACT)%NEXT_PED_ARRIVAL(PHASE) .NE. 0.) THEN
        IF(SIMTIME .GE. AC_SIGNALS(IACT)%NEXT_PED_ARRIVAL(PHASE)) THEN
          AC_SIGNALS(IACT)%PEDESTRIAN_DETECTOR_STATE(PHASE) = 1  
