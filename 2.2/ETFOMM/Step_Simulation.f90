...
! --- Update tram vehicles.
 
    IF(TRAM_SIMULATION) CALL UPDATE_TRAM_VEHICLES
...
! --- Move vehicles from storage.
  ...
  IF(N_FREEWAY_LINKS .GT. 0 .AND. N_STREET_LINKS .GT. 0) THEN
    CALL QSORT_VEHICLE_LIST
    DO INODE = 1, MAX_NODE_NUMBER
      IF(NODE_TYPE(INODE) .EQ. NT_INTERFACE) THEN
...
!LZ 07/09/2024 Save anamation data for pipe
