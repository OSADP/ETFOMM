...
! ----------------------------------------------------------------------
  ...
  RETURN
    ...
    ! ==================================================================================================
  SUBROUTINE ExportEntryVehicles
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
  ...
  IMPLICIT NONE
      ...
      ! Global  variables
    !TYPE(QPOINTER), DIMENSION(:), POINTER :: FRONT  ! Array of QPOINTER
    
    ! Local variables
    TYPE(QNODE), POINTER :: currentQNode
    INTEGER :: entryLinkID, globalID
    REAL :: departure
...
! ---        remove vehicle from queue and repeat for next vehicle in queue
    ...
    filename = "C:\\TSIS6 Projects\\Example Files\\vehicles.txt"
    OPEN(UNIT=iunit, FILE=filename, STATUS="UNKNOWN", ACTION="WRITE", POSITION="APPEND")
 
    DO IN = 1, NUMBER_OF_ENTRYNODES
        

    ! Check if the current simtime is a multiple of 60

        ! Traverse the linked list starting from FRONT(IN)%PTR
        currentQNode => FRONT(IN)%PTR
        DO WHILE (ASSOCIATED(currentQNode))

            ! Extract relevant vehicle data from the current node
            entryLinkID = currentQNode%VEHICLE%ENTRYLINKID
            departure   = currentQNode%VEHICLE%DEPARTURE
            globalID    = currentQNode%VEHICLE%GLOBALID

            ! Write the vehicle data to the file
            WRITE(iunit, '(I10, 2X, F10.2, 2X, I10)') entryLinkID, departure, globalID
...
! ---      if successfully emitted
! ---        remove vehicle from queue and repeat for next vehicle in queue
          TYPE(QNODE), POINTER :: tempPtr
 
  DO IN = 1, NUMBER_OF_ENTRYNODES
    DO
      IF(ASSOCIATED(FRONT(IN)%PTR)) THEN

! Assign FRONT(IN)%PTR to tempPtr for debugging
tempPtr => FRONT(IN)%PTR
...
! ----------------------------------------------------------------------
  ...
  IMPLICIT NONE
  TYPE(VEHICLE) :: FRONT_VEHICLE
  TYPE(QNODE), POINTER :: FRONT_PTR, TAIL_PTR, TEMP, tempPtr_PBV
  LOGICAL :: WEMIT
  INTEGER :: IV
! ----------------------------------------------------------------------
  DO
    IF(.NOT. ASSOCIATED(PBV_FRONT%PTR)) RETURN
    tempPtr_PBV=PBV_FRONT%PTR
...
! ----------------------------------------------------------------------
  DO NBR = 1, NUMBER_OF_ROUTES
    ...
    ITIME = INT(SIMTIME)
    IF((INITMODE .OR. SKIP_INIT) .AND. ITIME .EQ. 0) THEN
...
! ----------------------------------------------------------------------
 ...
 !LZ changed the distribution calculations     
 !   MU = 60. / NUMVEHS
    LAMUDA=NUMVEHS/60.
    
    ERLANGA=NUMVEHS
    !DO I = 1, NUMVEHS
      CALL ERL_DISTR(LAMUDA, ERLANGA, RHEAD)
    !ENDDO
...
! ==================================================================================================
  SUBROUTINE ERL_DISTR_OutDated(MU, K, X)
! ----------------------------------------------------------------------
!     Derive headways from the Erlang distribution.
!  This is the previous version and see updated version below
! ----------------------------------------------------------------------
  USE ENTRYNODE_DATA
  USE SIMPARAMS
  IMPLICIT NONE
...
! ----------------------------------------------------------------------
  ...
  RETURN
    ...
    ! ==================================================================================================
  SUBROUTINE ERL_DISTR(LAMUDA, L, X)
! ----------------------------------------------------------------------
!     Derive headways from the Erlang distribution.
!  This is Updated Verion by LZ 09/08/2024
! K sharpnest paramters , uses K=4
! Mu is the arrival rate in vehicles per second, mu=K/60  
! X is the headway array between consective vehilces, in seconds  
! ----------------------------------------------------------------------
  USE ENTRYNODE_DATA
  USE SIMPARAMS
  IMPLICIT NONE
  REAL, INTENT(IN) :: LAMUDA
  INTEGER, INTENT(IN) :: L
  REAL, INTENT(OUT) :: X (L)
  INTEGER :: I, J, K=4
! ----------------------------------------------------------------------

  REAL :: u, LAMUDA1, headway

    ! Generate Erlang-distributed values by summing exponential distributions
  K=4
  LAMUDA1=LAMUDA*K
    ...
    DO I = 1, L
        headway = 0.0
        DO J = 1, K
            CALL RANDOM_NUMBER(u)  ! Generate uniform random number
            headway = headway - LOG(u) / LAMUDA1
        END DO
        X(I)=headway
