...
! --- Vehicle arrays
    ...
    REAL,    ALLOCATABLE :: DISTANCE_TRAVELED(:)
    ...
    INTEGER, ALLOCATABLE :: TRAM_LEADER(:)
    logical, allocatable :: last_car_checked_out(:)
    logical, allocatable :: tram_follower(:)
    logical, allocatable :: last_tram_car(:)
    ...
    REAL,    ALLOCATABLE :: STATION_TRAVELTIME(:)
    ...
    LOGICAL, ALLOCATABLE :: WILL_BYPASS(:)
...
! ----------------------------------------------------------------------
    ...
    ALLOCATE(DISTANCE_TRAVELED(MAX_VEHICLES_S))
    DISTANCE_TRAVELED = 0.
    ...
    ALLOCATE(TRAM_LEADER(MAX_VEHICLES_S))
    TRAM_LEADER = 0
        
    ALLOCATE(last_car_checked_out(MAX_VEHICLES_S))
    last_car_checked_out = .false.
    
    ALLOCATE(tram_follower(MAX_VEHICLES_S))
    tram_follower = .false.
        
    ALLOCATE(last_tram_car(MAX_VEHICLES_S))
    last_tram_car = .false.
    ...
    ALLOCATE(WILL_BYPASS(MAX_VEHICLES_S))
    WILL_BYPASS = .FALSE.
    ...
    ALLOCATE(STATION_TRAVELTIME(MAX_VEHICLES_S))
    STATION_TRAVELTIME = 0.
...
! --- Process tram cars
    ...
    IF(SLINK(IV) .NE. 0) THEN
      IF(TRAM_LINK(SLINK(IV)) .AND. SROUTEID(IV) .NE. 0) THEN
        NBR = SROUTEID(IV)
        IF(BUSR_TRAMCARS(NBR) .GT. 1 .AND. .NOT. LAST_TRAM_CAR(IV)) THEN
          IVF = SFOLLOWER(IV)
          IF(IVF .NE. 0) TRAM_LEADER(IVF) = 0
          DO WHILE(IVF .NE. 0)
            SLOCATION(IVF) = SLOCATION(IVF) + DISTANCE_TRAVELED(IV)
            SSPEED(IVF) = SSPEED(IV)
            NEXT_STOP(IVF) = NEXT_STOP(IV)
            IF(LAST_TRAM_CAR(IVF)) EXIT
            IVF = SFOLLOWER(IVF)
          ENDDO
        ENDIF
      ENDIF
...
! ----------------------------------------------------------------------
    ...
    CALL REALLOCATE_REAL(DISTANCE_TRAVELED, I1, I2) 
    ...
    CALL REALLOCATE_INTEGER(TRAM_LEADER, I1, I2)
    CALL REALLOCATE_logical(last_car_checked_out, I1, I2)
    CALL REALLOCATE_logical(last_tram_car, I1, I2)
    CALL REALLOCATE_logical(tram_follower, I1, I2)
    ...
    CALL REALLOCATE_LOGICAL(WILL_BYPASS, I1, I2) 
    ...
    CALL REALLOCATE_REAL(STATION_TRAVELTIME, I1, I2)
...
! ==================================================================================================
    ...
    IF(ALLOCATED(DISTANCE_TRAVELED)) DEALLOCATE(DISTANCE_TRAVELED)
    ...
    IF(ALLOCATED(TRAM_LEADER)) DEALLOCATE(TRAM_LEADER)
    IF(ALLOCATED(last_car_checked_out)) DEALLOCATE(last_car_checked_out)
    IF(ALLOCATED(tram_follower)) DEALLOCATE(tram_follower)
    IF(ALLOCATED(last_tram_car)) DEALLOCATE(last_tram_car)
    ...
    IF(ALLOCATED(WILL_BYPASS)) DEALLOCATE(WILL_BYPASS)
    ...
    IF(ALLOCATED(STATION_TRAVELTIME)) DEALLOCATE(STATION_TRAVELTIME)
    ...
    IF(TOTAL .GT. 0) THEN
      DO IV = 1, HIGHEST_INDEX_S
        IF(SID(IV) .NE. 0) THEN
          ...
          WRITE(FILE) DISTANCE_TRAVELED(IV)
          ...
          WRITE(FILE) TRAM_LEADER(IV)
          WRITE(FILE) last_car_checked_out(IV)
          WRITE(FILE) tram_follower(IV)
          WRITE(FILE) last_tram_car(IV)
          ...
          WRITE(FILE) STATION_TRAVELTIME(IV)
...
! ----------------------------------------------------------------------
    ...
    IF(TOTAL .GT. 0) THEN
      DO I = 1, TOTAL
        ...
        READ(FILE) DISTANCE_TRAVELED(IV)
        ...
        READ(FILE) TRAM_LEADER(IV)
        READ(FILE) last_car_checked_out(IV)
        READ(FILE) tram_follower(IV)
        READ(FILE) last_tram_car(IV)
        ...
        READ(FILE) STATION_TRAVELTIME(IV)
