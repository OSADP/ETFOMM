...
#define DELTA_T 5 //TODO move to a better place //AC Phase control
...
struct FILENAME
{
	char str[MAX_FILE_NAME_LEN];//512
	int  len;
};

struct AC_INPUTS //verigfied with ETFOMM 06/16/2024 by LZ
{
	int node[8];							//AC node ID, changed to arry of 8 by LZ 06/16/24
	...
	// int ped_allowed[8];					//allow pedestrian 8 phases, NOT implemented per ETFOMM LZ 06/16/2024
	...
	int direct_approach_DSN[NUMBER_OF_AC_APPROACHES]; //added 06/16/2024
	...
	int checkin_dist[NUMBER_OF_AC_APPROACHES][8];  //added by LZ 06/16/2024
	int checkout_dist[NUMBER_OF_AC_APPROACHES][8];
	int policy[NUMBER_OF_AC_APPROACHES][8];
...
//veried with ETFOMM 06/16/2024 by LZ
struct FREEWAY_LINK
{
	int ID;
	int usn;
...
struct STREET_LINK
{
   ...
   int channelization[N_STREET_LANES];
	float lane_length[N_STREET_LANES];
   int leftpct;
   int thrupct;
   int rightpct;
   int diagpct;
...
struct BUSROUTE_DATA
{
	int number;
	int typeofroute;
    int number_of_tramcars; //added 06/16/2025
	int hdwy;
	int offset;
	int nodes;
	int route_nodes[200];
...
struct COND_TURNPCTS
{
	int USN;
	int DSN;
	int LEFTPCT[5]; //changed to 5 and added one more diag LZ 06/16/2024
	int THRUPCT[5];
	int RIGHTPCT[5];
	int DIAGPCT[5];
	int RDIAGPCT[5];
...
//verified LZ 06/16/2024 with ETFOMM
struct API_DETECTOR_DATA
{
	...
	int station_id;
	float location;
	int detector_lanes[N_DETLANE]; //added 06/16/2024 LZ

	//int lane1; //delted LZ: outdated
	//int lane2;
	int zone_length;
	float delay_time;
	float carryover_time;
