...
! --- Determine a lane to enter, if possible.
  ...
  FENTRYTIME(IV) = SIMTIME + MOD(THIS_VEHICLE%DEPARTURE, TIMESTEP) - TIMESTEP
  CALL FREEWAY_CHOOSE_ENTRY_LANE(IV, ILD)
  IF(FLANE(IV) .NE. 0) THEN
    IF(ILD .NE. 0) FENTRYTIME(IV) = MAX(FENTRYTIME(IV), FENTRYTIME(ILD) + MINSEP)
    ...
    IF(ILD .NE. 0) THEN
      ...
      IF(ACCEL .LT. 0.) FSPEED(IV) = MIN(FSPEED(IV), FSPEED(ILD))
    ENDIF
    FLOCATION(IV) = -MOD(FENTRYTIME(IV), TIMESTEP) * FSPEED(IV)
    FLEADER(IV) = ILD
...
! --- Determine a lane to enter, if possible.
  ...
  SENTRYTIME(IV) = SIMTIME + MOD(THIS_VEHICLE%DEPARTURE, TIMESTEP) - TIMESTEP
  CALL STREET_CHOOSE_ENTRY_LANE(IV, ILD)
  IF(SLANE(IV) .NE. 0) THEN
    IF(ILD .NE. 0) SENTRYTIME(IV) = MAX(SENTRYTIME(IV), SENTRYTIME(ILD) + MINSEP)
    ...
    IF(ILD .NE. 0) THEN
      ...
      IF(ACCEL .LT. 0.) SSPEED(IV) = MIN(SSPEED(IV), SSPEED(ILD))
    ENDIF
    SLOCATION(IV) = -MOD(SENTRYTIME(IV), TIMESTEP) * SSPEED(IV)
