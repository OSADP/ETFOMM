...
! ----------------------------------------------------------------------
  DO I = 1, N_FREEWAY_DETECTORS
    ...
    IF(FDETECTOR(I)%ZONE_LENGTH .LT. 0 .OR. FDETECTOR(I)%ZONE_LENGTH .GT. 50) THEN
      ...
      CALL SENDTEXTMSG(M_ERROR)
    ENDIF
    IF(FDETECTOR(I)%TYPE_CODE .NE. 1) THEN
      CALL NEW_ERROR
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_RAMPMETERS
    ...
    IF(RAMPMETERS(I)%CONTROL .EQ. 1) THEN
      IF(RAMPMETERS(I)%TWO_PERGREEN) THEN
        IF(RAMPMETERS(I)%HEADWAY(1) .LT. 5.0 .OR. RAMPMETERS(I)%HEADWAY(1) .GT. 60.0) THEN
          CALL NEW_ERROR
          WRITE(MSGTEXT, '(A, I4)') 'INVALID HEADWAY FOR RAMPMETER AT NODE ',  RAMPMETERS(I)%DSN
          CALL SENDTEXTMSG(M_ERROR)
          WRITE(MSGTEXT, '(A, F5.1)') '  HEADWAY = ', RAMPMETERS(I)%HEADWAY(1)
          CALL SENDTEXTMSG(M_ERROR)
        ENDIF
      ELSE
        IF(RAMPMETERS(I)%HEADWAY(1) .LT. 2.0 .OR. RAMPMETERS(I)%HEADWAY(1) .GT. 60.0) THEN
          CALL NEW_ERROR
          WRITE(MSGTEXT, '(A, I4)') 'INVALID HEADWAY FOR RAMPMETER AT NODE ',  RAMPMETERS(I)%DSN
          CALL SENDTEXTMSG(M_ERROR)
          WRITE(MSGTEXT, '(A, F5.1)') '  HEADWAY = ', RAMPMETERS(I)%HEADWAY(1)
    ...
    ELSEIF(RAMPMETERS(I)%CONTROL .EQ. 2) THEN
      IF(RAMPMETERS(I)%CAPACITY .LT. 0) THEN
        CALL NEW_ERROR
        WRITE(MSGTEXT, '(A, I4)') 'NEGATIVE CAPACITY FOR RAMPMETER AT NODE ',  RAMPMETERS(I)%DSN
        CALL SENDTEXTMSG(M_ERROR)
        WRITE(MSGTEXT, '(A, I1, A, F10.1)') '  ENTRY = ', I, 'VALUE = ', RAMPMETERS(I)%CAPACITY
        CALL SENDTEXTMSG(M_ERROR)
      ENDIF
      
    ELSEIF(RAMPMETERS(I)%CONTROL .EQ. 3) THEN
...
! ----------------------------------------------------------------------
  DO I = 1, NUMBER_OF_INCIDENTS
    ...
    IF(INCIDENT_BEGIN_POINT(I) .LT. 0 .OR. INCIDENT_BEGIN_POINT(I) .GT. FLENGTH(IL)) THEN
      ...
      CALL SENDTEXTMSG(M_ERROR)
    ENDIF
    IF(INCIDENT_END_POINT(I) .LT. 0 .OR. INCIDENT_END_POINT(I) .GT. FLENGTH(IL)) THEN
      CALL NEW_ERROR
      WRITE(MSGTEXT, '(A, I4)') 'INVALID END LOCATION FOR INCIDENT NUMBER ', I
      CALL SENDTEXTMSG(M_ERROR)
      WRITE(MSGTEXT, '(A, I4)') ' END LOCATION = ', INCIDENT_END_POINT(I)
      CALL SENDTEXTMSG(M_ERROR)
    ENDIF
...
! ----------------------------------------------------------------------
  DO IL = 1, N_STREET_LINKS
    ...
    ELSE
      ...
      !IF(SLENGTH(IL) .LT. 50) THEN
      !  CALL NEW_ERROR
      !  WRITE(MSGTEXT, '(A, 2I5)') 'INVALID LENGTH SPECIFIED FOR INTERNAL LINK ', IUP, IDOWN
      !  CALL SENDTEXTMSG(M_ERROR)
      !  WRITE(MSGTEXT, '(A, I4)') '  LENGTH = ', SLENGTH(IL)
      !  CALL SENDTEXTMSG(M_ERROR)
      !ENDIF
...
!Check for required receiving links and illogical channelization.
      ...
      DO I = FIRST_FULL_LANE(IL), LAST_FULL_LANE(IL)
        ...
        IF(ICHAN .EQ. 1 .OR. ICHAN .EQ. 8) THEN
          IF(LEFT_LINK(IL) .EQ. 0) THEN
            CALL NEW_ERROR
        ...
        ELSEIF(ICHAN .EQ. 4 .OR. ICHAN .EQ. 7) THEN
          IF(RIGHT_LINK(IL) .EQ. 0) THEN
            CALL NEW_ERROR
  ...
  DO IBR = 1, NUMBER_OF_ROUTES   
    ...
    ELSEIF(BUSR_NNODES(IBR) .NE. 0) THEN
      ...
      DO J = 1, BUSR_NNODES(IBR)
        IF(BUSR_ROUTE_NODES(IBR, J) .LT. 0 .OR. BUSR_ROUTE_NODES(IBR, J) .GT. 8999) THEN
          NERR = 1
          CALL NEW_ERROR
